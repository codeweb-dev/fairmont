@foreach ($reports as $report)
    ...
    <table>
        ...
        <tr><td>Unit:</td><td>{{ $report->unit->name ?? 'N/A' }}</td></tr>
        ...
        <tr><td>HSFO:</td><td>{{ $recv->hsfo ?? 'N/A' }}</td></tr>
        <tr><td>VLSFO:</td><td>{{ $recv->vlsfo ?? 'N/A' }}</td></tr>
        <tr><td>BIOFUEL:</td><td>{{ $recv->biofuel ?? 'N/A' }}</td></tr>
        <tr><td>LSMGO:</td><td>{{ $recv->lsmgo ?? 'N/A' }}</td></tr>
        <tr><td>ME CC OIL:</td><td>{{ $recv->me_cc_oil ?? 'N/A' }}</td></tr>
        <tr><td>MC CYL OIL:</td><td>{{ $recv->mc_cyl_oil ?? 'N/A' }}</td></tr>
        <tr><td>GE CC OIL:</td><td>{{ $recv->ge_cc_oil ?? 'N/A' }}</td></tr>
        <tr><td>FW:</td><td>{{ $recv->fw ?? 'N/A' }}</td></tr>
        <tr><td>FW Produced:</td><td>{{ $recv->fw_produced ?? 'N/A' }}</td></tr>

        <tr><td colspan="7"></td></tr>

        {{-- Consumption --}}
        <tr><td colspan="2" style="font-weight: bold;">Consumption</td></tr>
        <tr><td>HSFO:</td><td>{{ $cons->hsfo ?? 'N/A' }}</td></tr>
        <tr><td>VLSFO:</td><td>{{ $cons->vlsfo ?? 'N/A' }}</td></tr>
        <tr><td>BIOFUEL:</td><td>{{ $cons->biofuel ?? 'N/A' }}</td></tr>
        <tr><td>LSMGO:</td><td>{{ $cons->lsmgo ?? 'N/A' }}</td></tr>
        <tr><td>ME CC OIL:</td><td>{{ $cons->me_cc_oil ?? 'N/A' }}</td></tr>
        <tr><td>MC CYL OIL:</td><td>{{ $cons->mc_cyl_oil ?? 'N/A' }}</td></tr>
        <tr><td>GE CC OIL:</td><td>{{ $cons->ge_cc_oil ?? 'N/A' }}</td></tr>
        <tr><td>FW:</td><td>{{ $cons->fw ?? 'N/A' }}</td></tr>
        <tr><td>FW Produced:</td><td>{{ $cons->fw_produced ?? 'N/A' }}</td></tr>
