            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm">
                            {{ \Carbon\Carbon::parse($selectedReport->port_etd)->format('M d, Y h:i A') }}
                        </p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-sm">
                            {{ \Carbon\Carbon::parse($selectedReport->bunker_completed)->format('M d, Y h:i A') }}
                        </p>
                    </div>
                    <div>
                        <flux:label>Bunker GMT Offset</flux:label>
                        <p class="text-sm">{{ $selectedReport->bunker_gmt_offset }}</p>
                    </div>
                </div>

                <flux:separator />

                @if ($selectedReport->bunker)
                    <div class="grid grid-cols-2 gap-4 pt-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm">
                                {{ $selectedReport->bunker->hsfo_quantity !== null ? rtrim(rtrim(number_format((float) $selectedReport->bunker->hsfo_quantity, 3, '.', ''), '0'), '.') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>HSFO Viscosity</flux:label>
                            <p class="text-sm">{{ $selectedReport->bunker->hsfo_viscosity }}</p>
                        </div>

                        <div>
                            <flux:label>BIOFUEL (MT)</flux:label>
                            <p class="text-sm">
                                {{ $selectedReport->bunker->biofuel_quantity !== null ? rtrim(rtrim(number_format((float) $selectedReport->bunker->biofuel_quantity, 3, '.', ''), '0'), '.') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>BIOFUEL Viscosity</flux:label>
                            <p class="text-sm">{{ $selectedReport->bunker->biofuel_viscosity }}</p>
                        </div>

                        <div>
                            <flux:label>VLSFO (MT)</flux:label>
                            <p class="text-sm">
                                {{ $selectedReport->bunker->vlsfo_quantity !== null ? rtrim(rtrim(number_format((float) $selectedReport->bunker->vlsfo_quantity, 3, '.', ''), '0'), '.') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>VLSFO Viscosity</flux:label>
                            <p class="text-sm">{{ $selectedReport->bunker->vlsfo_viscosity }}</p>
                        </div>

                        <div>
                            <flux:label>LSMGO (MT)</flux:label>
                            <p class="text-sm">
                                {{ $selectedReport->bunker->lsmgo_quantity !== null ? rtrim(rtrim(number_format((float) $selectedReport->bunker->lsmgo_quantity, 3, '.', ''), '0'), '.') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>LSMGO Viscosity</flux:label>
                            <p class="text-sm">{{ $selectedReport->bunker->lsmgo_viscosity }}</p>
                        </div>
                    </div>
                @endif
                ...
                @if ($selectedReport->assiociated_information)
                    <div class="grid grid-cols-2 gap-4 pt-4">
                        <flux:heading size="sm" class="col-span-2">Associated Info</flux:heading>

                        <div>
                            <flux:label>Port Delivery</flux:label>
                            <p class="text-sm">{{ $selectedReport->assiociated_information->port_delivery }}</p>
                        </div>
                        <div>
                            <flux:label>EOSP</flux:label>
                            <p class="text-sm">
                                {{ $selectedReport->assiociated_information->eosp ? \Carbon\Carbon::parse($selectedReport->assiociated_information->eosp)->format('M d, Y h:i A') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>EOSP GMT</flux:label>
                            <p class="text-sm">{{ $selectedReport->assiociated_information->eosp_gmt }}</p>
                        </div>
                        <div>
                            <flux:label>Barge Alongside</flux:label>
                            <p class="text-sm">
                                {{ $selectedReport->assiociated_information->barge ? \Carbon\Carbon::parse($selectedReport->assiociated_information->barge)->format('M d, Y h:i A') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>Barge GMT</flux:label>
                            <p class="text-sm">{{ $selectedReport->assiociated_information->barge_gmt }}</p>
                        </div>
                        <div>
                            <flux:label>COSP</flux:label>
                            <p class="text-sm">
                                {{ $selectedReport->assiociated_information->cosp ? \Carbon\Carbon::parse($selectedReport->assiociated_information->cosp)->format('M d, Y h:i A') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>COSP GMT</flux:label>
                            <p class="text-sm">{{ $selectedReport->assiociated_information->cosp_gmt }}</p>
                        </div>
                        <div>
                            <flux:label>Anchor Dropped</flux:label>
                            <p class="text-sm">
                                {{ $selectedReport->assiociated_information->anchor ? \Carbon\Carbon::parse($selectedReport->assiociated_information->anchor)->format('M d, Y h:i A') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>Anchor GMT</flux:label>
                            <p class="text-sm">{{ $selectedReport->assiociated_information->anchor_gmt }}</p>
                        </div>
                        <div>
                            <flux:label>Pumping Completed</flux:label>
                            <p class="text-sm">
                                {{ $selectedReport->assiociated_information->pumping ? \Carbon\Carbon::parse($selectedReport->assiociated_information->pumping)->format('M d, Y h:i A') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>Pumping GMT</flux:label>
                            <p class="text-sm">{{ $selectedReport->assiociated_information->pumping_gmt }}</p>
                        </div>
                    </div>
                @endif
