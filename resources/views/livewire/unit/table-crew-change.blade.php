<div>
    <div class="flex flex-col gap-6 mb-6">
        ...
        <div class="flex gap-3 justify-end items-center w-full">
            @php
                $activeSelectedReports = $selectedCrewChange ?? [];
            @endphp
    ...
    <x-admin-components.table>
        ...
        @foreach ($reports as $report)
            <tr class="hover:bg-white/5 bg-black/5 transition-all" wire:key="crew-change-row-{{ $report->id }}">
                <td class="px-3 py-4">
                    <flux:checkbox wire:model.live="selectedCrewChange" value="{{ $report->id }}" />
                </td>
                <td class="px-3 py-4">{{ $report->report_type }}</td>
                <td class="px-3 py-4">{{ $report->vessel->name }}</td>
                <td class="px-3 py-4">Crew Change</td>
                <td class="px-3 py-4">
                ...
                <td class="px-3 py-4 flex items-center gap-2">
                    ...
                    <flux:modal name="view-report-{{ $report->id }}" class="min-w-[28rem] md:w-[48rem]"
                        wire:key="crew-change-view-modal-{{ $report->id }}">
                        <div class="space-y-6">
                    ...
                    <flux:modal name="delete-report-{{ $report->id }}" class="min-w-[22rem]"
                        wire:key="crew-change-delete-modal-{{ $report->id }}">
                        <div class="space-y-6">
                            <div>
                                <flux:heading size="lg">Soft Delete Report?</flux:heading>
                                <flux:text class="mt-2">
                                    Are you sure you want to delete the Crew Monitoring Plan Report? <br> This report
                                    will not be permanently deleted and can be restored if needed.
                                </flux:text>
                            </div>
