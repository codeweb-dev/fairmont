    ...
    @if ($showModal && $selectedReport)
        <flux:modal name="report-details-modal" class="max-w-screen" wire:model="showModal">
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <div>
                            ...
                            <p>{{ $selectedReport->noon_report->cp_ordered_speed ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Allowed M/E Cons. at C/P Speed</flux:label>
                            <p>{{ $selectedReport->noon_report->me_cons_cp_speed ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Obs. Distance (NM)</flux:label>
                            <p>{{ $selectedReport->noon_report->obs_distance ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Steaming Time (Hrs)</flux:label>
                            <p>{{ $selectedReport->noon_report->steaming_time ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Avg Speed (Kts)</flux:label>
                            <p>{{ $selectedReport->noon_report->avg_speed ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Distance to go (NM)</flux:label>
                            <p>{{ $selectedReport->noon_report->distance_to_go ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Course (Deg)</flux:label>
                            <p>{{ $selectedReport->noon_report->course ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Breakdown (Hrs)</flux:label>
                            <p>{{ $selectedReport->noon_report->breakdown ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Avg RPM</flux:label>
                            <p>{{ $selectedReport->noon_report->avg_rpm ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Engine Distance (NM)</flux:label>
                            <p>{{ $selectedReport->noon_report->engine_distance ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Slip (%)</flux:label>
                            <p>{{ $selectedReport->noon_report->slip ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>M/E Output (% MCR)</flux:label>
                            <p>{{ $selectedReport->noon_report->me_output_mcr ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Avg Power (kW)</flux:label>
                            <p>{{ $selectedReport->noon_report->avg_power ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Logged Distance (NM)</flux:label>
                            <p>{{ $selectedReport->noon_report->logged_distance ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Speed Through Water (Kts)</flux:label>
                            <p>{{ $selectedReport->noon_report->speed_through_water ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Next Port</flux:label>
                            <p>{{ $selectedReport->noon_report->next_port ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>ETA Next Port</flux:label>
                            <p>{{ $selectedReport->noon_report->eta_next_port ? \Carbon\Carbon::parse($selectedReport->noon_report->eta_next_port)->format('M d, Y h:i A') : '' }}
                            </p>
                        </div>
                        <div>
                            <flux:label>ETA GMT Offset</flux:label>
                            <p>{{ $selectedReport->noon_report->eta_gmt_offset ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Anchored Hours</flux:label>
                            <p>{{ $selectedReport->noon_report->anchored_hours ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Drifting Hours</flux:label>
                            <p>{{ $selectedReport->noon_report->drifting_hours ?? '' }}</p>
                        </div>
                        <div>
                            <flux:label>Maneuvering Hours</flux:label>
                            <p>{{ $selectedReport->noon_report->maneuvering_hours ?? '' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-4">
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <div>
                            ...
                            <p>{{ $selectedReport->noon_report->condition }}</p>
                        </div>
                        <div>
                            <flux:label>Displacement (MT)</flux:label>
                            <p>{{ $selectedReport->noon_report->displacement }}</p>
                        </div>
                        <div>
                            <flux:label>Cargo Name</flux:label>
                            <p>{{ $selectedReport->noon_report->cargo_name }}</p>
                        </div>
                        <div>
                            <flux:label>Cargo Weight (MT)</flux:label>
                            <p>{{ $selectedReport->noon_report->cargo_weight }}</p>
                        </div>
                        <div>
                            <flux:label>Ballast Weight (MT)</flux:label>
                            <p>{{ $selectedReport->noon_report->ballast_weight }}</p>
                        </div>
                        <div>
                            <flux:label>Fresh Water (MT)</flux:label>
                            <p>{{ $selectedReport->noon_report->fresh_water }}</p>
                        </div>
                        <div>
                            <flux:label>Fwd Draft (m)</flux:label>
                            <p>{{ $selectedReport->noon_report->fwd_draft }}</p>
                        </div>
                        <div>
                            <flux:label>Aft Draft (m)</flux:label>
                            <p>{{ $selectedReport->noon_report->aft_draft }}</p>
                        </div>
                        <div>
                            <flux:label>GM</flux:label>
                            <p>{{ $selectedReport->noon_report->gm }}</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Via -->
                @isset($selectedReport->port_gmt_offset)
                    @if (trim($selectedReport->port_gmt_offset) === 'At Sea')
                        <!-- Via -->
                        <div class="pt-4">
                            ...
                            <div class="grid grid-cols-4 gap-4">
                                <div>
                                    <flux:label>Next Port</flux:label>
                                    <p>{{ $selectedReport->noon_report->next_port_voyage }}</p>
                                </div>
                                <div>
                                    <flux:label>Via</flux:label>
                                    <p>{{ $selectedReport->noon_report->via }}</p>
                                </div>
                                <div>
                                    <flux:label>ETA (LT)</flux:label>
                                    <p>{{ $selectedReport->noon_report->eta_lt }}</p>
                                </div>
                                <div>
                                    <flux:label>GMT Offset</flux:label>
                                    <p>{{ $selectedReport->noon_report->gmt_offset_voyage }}</p>
                                </div>
                                <div>
                                    <flux:label>Distance to go</flux:label>
                                    <p>{{ $selectedReport->noon_report->distance_to_go_voyage }}</p>
                                </div>
                                <div>
                                    <flux:label>Projected Speed (kts)</flux:label>
                                    <p>{{ $selectedReport->noon_report->projected_speed }}</p>
                                </div>
                            </div>
                        </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <div>
                            ...
                            <p>{{ $selectedReport->noon_report->wind_force_average_weather }}</p>
                        </div>
                        <div>
                            <flux:label>Swell</flux:label>
                            <p>{{ $selectedReport->noon_report->swell }}</p>
                        </div>
                        <div>
                            <flux:label>Sea Current (Kts) (Rel.)</flux:label>
                            <p>{{ $selectedReport->noon_report->sea_current }}</p>
                        </div>
                        <div>
                            <flux:label>Sea Temp (Deg. °C)</flux:label>
                            <p>{{ $selectedReport->noon_report->sea_temp }}</p>
                        </div>
                        <div>
                            <flux:label>Observed Wind Dir. (T)</flux:label>
                            <p>{{ $selectedReport->noon_report->observed_wind }}</p>
                        </div>
                        <div>
                            <flux:label>Wind Sea Height (m)</flux:label>
                            <p>{{ $selectedReport->noon_report->wind_sea_height }}</p>
                        </div>
                        <div>
                            <flux:label>Sea Current Direction. (Rel.)</flux:label>
                            <p>{{ $selectedReport->noon_report->sea_current_direction }}</p>
                        </div>
                        <div>
                            <flux:label>Swell Height (m)</flux:label>
                            <p>{{ $selectedReport->noon_report->swell_height }}</p>
                        </div>
                        <div>
                            <flux:label>Observed Sea Dir. (T)</flux:label>
                            <p>{{ $selectedReport->noon_report->observed_sea }}</p>
                        </div>
                        <div>
                            <flux:label>Air Temp (Deg. °C)</flux:label>
                            <p>{{ $selectedReport->noon_report->air_temp }}</p>
                        </div>
                        <div>
                            <flux:label>Observed Swell Dir. (T)</flux:label>
                            <p>{{ $selectedReport->noon_report->observed_swell }}</p>
                        </div>
                        <div>
                            <flux:label>Sea DS</flux:label>
                            <p>{{ $selectedReport->noon_report->sea_ds }}</p>
                        </div>
                        <div>
                            <flux:label>Atm. Pressure (millibar)</flux:label>
                            <p>{{ $selectedReport->noon_report->atm_pressure }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <div>
                            ...
                            <p>{{ $selectedReport->noon_report->wind_force_previous }}</p>
                        </div>
                        <div>
                            <flux:label>Wind Force (Bft.) (continuous)</flux:label>
                            <p>{{ $selectedReport->noon_report->wind_force_current }}</p>
                        </div>
                        <div>
                            <flux:label>Sea State (DS) >0 hrs (since last report)</flux:label>
                            <p>{{ $selectedReport->noon_report->sea_state_previous }}</p>
                        </div>
                        <div>
                            <flux:label>Sea State (continuous)</flux:label>
                            <p>{{ $selectedReport->noon_report->sea_state_current }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm border-collapse border border-zinc-200 dark:border-zinc-700">
                            <thead>
                                <tr>
                                    <th class="p-2 border border-zinc-200 dark:border-zinc-700">Time Block
                                    </th>
                                    <th class="p-2 border border-zinc-200 dark:border-zinc-700">Wind Force
                                    </th>
                                    <th class="p-2 border border-zinc-200 dark:border-zinc-700">Wind Dir
                                    </th>
                                    <th class="p-2 border border-zinc-200 dark:border-zinc-700">Swell Height
                                    </th>
                                    <th class="p-2 border border-zinc-200 dark:border-zinc-700">Swell Dir
                                    </th>
                                    <th class="p-2 border border-zinc-200 dark:border-zinc-700">Wind Sea
                                        Height</th>
                                    <th class="p-2 border border-zinc-200 dark:border-zinc-700">Sea Dir</th>
                                    <th class="p-2 border border-zinc-200 dark:border-zinc-700">Sea DS</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($selectedReport->weather_observations ?? [] as $block)
                                    <tr>
                ...
                <div>
                    ...
                    @foreach ($selectedReport->rob_tanks->groupBy('grade') as $grade => $tanks)
                        <p class="font-semibold mt-4">{{ $grade }}</p>
                        <table class="w-full text-sm border-collapse border border-zinc-200 dark:border-zinc-700 mb-4">
                            <thead>
                                <tr>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            ...
                            <p>{{ $selectedReport->noon_report->dg1_run_hours }}</p>
                        </div>
                        <div>
                            <flux:label>DG2 Run Hours</flux:label>
                            <p>{{ $selectedReport->noon_report->dg2_run_hours }}</p>
                        </div>
                        <div>
                            <flux:label>DG3 Run Hours</flux:label>
                            <p>{{ $selectedReport->noon_report->dg3_run_hours }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm whitespace-pre-line">{{ $selectedReport->master_info->master_info ?? '-' }}
                    </p>
                </div>
